...
locals {
  api_image = "us-central1-docker.pkg.dev/ticblue-pltndscrl-prod/ticblue-pltndscrl-prod-registry/ticblue_dev-uvlpic-backend:latest"
  fe_image  = "us-central1-docker.pkg.dev/ticblue-pltndscrl-prod/ticblue-pltndscrl-prod-registry/ticblue_dev-uvlpic-chat:latest"
}
...
# Input variable to define the SQL files to import
variable "sql_files" {
  type    = list(string)
  default = ["backup_industriacircularqa_16052023.sql"]  # Update with your SQL file names
}

# Import SQL files into the databases
resource "google_sql_database" "database" {
  count    = length(var.sql_files)
  name     = var.sql_files[count.index]
  instance = google_sql_database_instance.main.name

  charset = "UTF8"

  depends_on = [
    google_storage_bucket_object.sql_files,
  ]

  lifecycle {
    ignore_changes = [charset]
  }
}
...
# Import SQL files from Cloud Storage into the databases
resource "google_storage_bucket_object" "sql_files" {
  count  = length(var.sql_files)
  name   = var.sql_files[count.index]
  bucket = "ticblue-pltndscrl-prod-storage"
  source = "${var.sql_files[count.index]}"  # Path to download the file locally

  # Trigger the download when the bucket object is created
  provisioner "local-exec" {
    command = "gsutil cp gs://${google_storage_bucket_object.sql_files[count.index].bucket}/${google_storage_bucket_object.sql_files[count.index].name} ${google_storage_bucket_object.sql_files[count.index].source}"
  }
